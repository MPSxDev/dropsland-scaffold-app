...
export interface Client {
  /**
   * Construct and simulate a award_item transaction. Returns an `AssembledTransaction` object which will have a `result` field containing the result of the simulation. If this transaction changes contract state, you will need to call `signAndSend()` on the returned object.
   */
  award_item: (
    { to }: { to: string },
    options?: {
...
export class Client extends ContractClient {
  static async deploy<T = Client>(
    /** Constructor/Initialization Args for the contract's `__constructor` method */
    {
      ...
      name,
      symbol,
    }: { owner: string; base_uri: string; name: string; symbol: string },
    ...
    return ContractClient.deploy({ owner, base_uri, name, symbol }, options);
  }
  constructor(public readonly options: ContractClientOptions) {
    super(
      new ContractSpec([
        "AAAAAAAAAAAAAAANX19jb25zdHJ1Y3RvcgAAAAAAAAQAAAAAAAAABW93bmVyAAAAAAAAEwAAAAAAAAAIYmFzZV91cmkAAAAQAAAAAAAAAARuYW1lAAAAEAAAAAAAAAAGc3ltYm9sAAAAAAAQAAAAAA==",
        "AAAAAAAAAAAAAAAKYXdhcmRfaXRlbQAAAAAAAQAAAAAAAAACdG8AAAAAABMAAAABAAAABA==",
  ...
  public readonly fromJSON = {
    award_item: this.txFromJSON<u32>,
